...
def load_data(raw_dir):
    ...
    for key, value in data.items():
        data[key] = [
            os.path.join(value, folders[i], f"rqa_analysis_{folders[i]}_metrics.npz")
            for i in range(4)
        ]

    # Get window sizes from the first file
    window_sizes = np.load(data[list(data.keys())[0]][0], allow_pickle=True)[
        "window_sizes"
    ]
    ...
    for w in window_sizes:
        ...
        for electrode, files in data.items():
            for file_path in files:
                ...
                metrics = {"metrics": npz_data[metric_key]}
    ...
    for window in os.listdir(electrode_indexed_dir):
        ...
        for updown in os.listdir(window_path):
            ...
            for electrode_file in os.listdir(updown_path):
                ...
                if (
                    os.path.isfile(electrode_path)
                    and electrode_path not in dataset[window][updown][electrode]
                ):
